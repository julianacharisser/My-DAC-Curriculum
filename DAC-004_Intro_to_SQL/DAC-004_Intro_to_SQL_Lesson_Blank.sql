...
SELECT 
    product.productid,
    product.name AS productname,
    COALESCE(SUM(salesorderdetail.orderqty), 0) AS totalsalesquantity -- total sales quantity
FROM production.product AS product
...
-- My answer
SELECT firstname, middlename, lastname, emailaddress -- add the table name in front and use CONCAT
FROM humanresources.employee AS employee
	LEFT JOIN person.person AS person 
		ON employee.businessentityid = person.businessentityid
	LEFT JOIN person.emailaddress AS email
		ON employee.businessentityid = email.businessentityid; -- primary left table is the employee because
...
-- My answer
SELECT customer.customerid, person.firstname, 
FROM person.person AS person
	LEFT JOIN sales.customer AS customer
		ON person.businessentityid = customer.customerid
	LEFT JOIN sales.salesorderheader AS salesorder
		ON person.businessentityid = salesorder.customerid;
WHERE -- customers who never placed an order
...
FROM sales.customer AS customer
	LEFT JOIN person.person AS person -- primary left table is customer not person
		ON customer.personid = person.businessentityid
	LEFT JOIN sales.salesorderheader AS salesorderheader 
		ON customer.customerid = salesorderheader.customerid
GROUP BY -- ** this is where i am not sure what to put
	customer.customerid,
	person.firstname
...
SELECT 
	...
	lastname, 
	CONCAT(firstname, ' ', middlename, ' ', lastname) AS fullname,
	'Employee' AS category -- in the category column, we will hardcode 'Employee' for all the values in this part of the union
FROM person.person AS person
...
SELECT 
	...
	CASE 
		...
		ELSE 'Premium'
	END -- END is the marker that closes the CASE expression
	AS pricecategory -- the CASE expression will be in the pricecategory column
FROM production.product;
